...
function Restaurant({ list }) {
  const slideCard = list.filter((a) => a.resultList.partName === "식음료");
  // const areaName = list[0].resultList.areaName
  const encodedAreaName = encodeURIComponent(list[0].resultList.areaName);
  const encodedPartName = encodeURIComponent("식음료");
  console.log(list[0].resultList.areaName);
  return (
    <section className="Restaurant_frame">
      <div className="Restaurant_wrap mw">
        <div className="Restaurant_banner">
          ...
          <img src={`${process.env.PUBLIC_URL}/img/sec2_1.png`} alt="" />
        </div>
        <div className="Restaurant_content">
          <div className="Restaurant_detail">
            <h1 className="Restaurant_title">
              <img
                src={`${process.env.PUBLIC_URL}/img/sec2_icon1.png`}
                alt=""
              />
              식음료
            </h1>
          ...
          <p>반려동물과 함께 할 수 있는 식당과 카페들을 소개합니다.</p>
          <div className="Restaurant_cards">
            {slideCard.length > 0 ? slideCard.slice(0, 3).map((item, i) => {
  const image = item.resultList.imageList && item.resultList.imageList.length > 0 ? item.resultList.imageList[0].image : null;
  const defaultImage = `${process.env.PUBLIC_URL}/img/NoImage.png`;
  const keywords = item.resultList.keyword.split(",").map((word) => word.trim());
  return (
    <div className="card">
      <Link to={`/Detail/${item.resultList.contentSeq}`} className="card_link" />
      <div className="cardImg">
        <img src={image || defaultImage} alt={item.resultList.title} />
      </div>
